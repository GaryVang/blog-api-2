...
const signInUser = (db, bcrypt) => (req, res) => {
    ...
    .then(qResult => {
        ...
        bcrypt.compare(PASSWORD_TEXT, PASSWORD_HASH, function(err, cResult) {
            if(cResult){
                ...
                res.status(200).json({status: true, user_id: qResult.user_id, username: USERNAME, comment: USERNAME + ' SignIn Successful'});
            } else {
                res.status(400).json({status: false, comment:'SignIn Failed: Wrong Password'});
            }
        });
    ...
    .catch(error => {
        ...
        res.status(400).json({status: false, comment:'Error: User does not exist.'});
    });
};
...
const getPosts = (db) => (req, res) => {
    ...
    db.many('SELECT username, post_id, title, body, to_char(post_date, $3) as post_date FROM posts INNER JOIN users ON posts.user_id=users.user_id ORDER BY post_date desc LIMIT 5 OFFSET $1', 
...
const submitPost = (db) => (req, res) => {
    ...
    db.none('INSERT INTO posts(title, body, user_id) VALUES($1, $2, $3)', [TITLE, CONTENT, USER_ID])
        .then(() => {
            // success;
            // res.json('Post Submition Successful');
            res.status(200).json({status: true, comment: 'Post Submition Successful'});
